...
// Insert record in linked list 
    // Record* insertRecord (Record* head, char* citizenID, char* fName, char* lName, char* country, int age, char* virus, Date vaccDate) {
    //     Record* newNode;
...
Record* insertSortedRecord (Record* head, char* citizenID, char* fName, char* lName, char* country, int age, char* virus, Date vaccDate) {
    ...
    if (!head || strcmp(head->citizenID, citizenID)) {
        newNode->next = head;
        head = newNode;
    }
    // Move across list
